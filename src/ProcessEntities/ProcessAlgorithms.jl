...
This is primarily meant for the bootstrap call style generated by `@StatefulAlgorithm`,
for example:
...
function _pa_extract_processalgorithm_parts(ex)
    ...
    elseif _pa_is_config_macro(ex)
        ...
        isempty(args) && error("@config requires at least one field and a function definition when used with @StatefulAlgorithm.")
        length(args) >= 2 || error("@config used with @StatefulAlgorithm must wrap a function definition.")
    ...
    elseif ex isa Expr && ex.head == :block
        ...
        length(function_defs) == 1 || error("@StatefulAlgorithm block form requires exactly one function definition.")
        ...
        for stmt in statements
            ...
            elseif _pa_is_config_macro(stmt)
                ...
                error("@StatefulAlgorithm block form only supports `@config ...` statements plus one function definition. Got `$stmt`.")
            end
        end

        return only(function_defs), config_fields
    end
    error("@StatefulAlgorithm expects a function definition or a block containing `@config` declarations and one function definition.")
end
...
function _pa_parse_signature(call_ex)
    ...
    call_ex isa Expr && call_ex.head == :call || error("@StatefulAlgorithm expects a function definition")
    ...
    if !isnothing(params)
        for (idx, arg) in enumerate(params.args)
            if _pa_is_inputs_macro(arg)
                idx == length(params.args) || error("@input/@inputs/@init must be the last keyword-like argument in @StatefulAlgorithm")
                isempty(input_fields) || error("Only one @input/@inputs/@init block is allowed in @StatefulAlgorithm")
...
"""
    @StatefulAlgorithm function Name(...)
        ...
    end

Define a `StatefulAlgorithm` from a function-like signature.
...
```julia
@StatefulAlgorithm function Accumulate(x)
    return (; x = x + 1)
end
...
```julia
@StatefulAlgorithm function F(a, b)
    ...
end
```
...
```julia
@StatefulAlgorithm function F(a; gain = 1.0, offset::Float64 = 0.0)
    ...
end
```
...
- `@config` may be written in the surrounding block form or as a prelude like
  `@StatefulAlgorithm @config seed = 1 function F(...) ... end`
...
```julia
@StatefulAlgorithm function MyAlgo(...)
    body
end
```
...
- `struct MyAlgo <: StatefulAlgorithms.StatefulAlgorithm end`, or `Base.@kwdef struct MyAlgo ... end`
...
"""
function _stateful_algorithm_macro(ex)
    ...
    length(unique(direct_scope_names)) == length(direct_scope_names) || error("Duplicate names are not allowed in @StatefulAlgorithm: $(direct_scope_names)")
    ...
    struct_def = if isempty(config_fields)
        :(Base.@__doc__ struct $fname <: StatefulAlgorithms.StatefulAlgorithm end)
    else
        field_exprs = [Expr(:(=), _pa_binding_expr(field.name, field.typeexpr), field.default) for field in config_fields]
        :(Base.@__doc__ Base.@kwdef struct $fname <: StatefulAlgorithms.StatefulAlgorithm
            $(field_exprs...)
        end)
    end
...
macro StatefulAlgorithm(ex)
    return _stateful_algorithm_macro(ex)
end

"""Deprecated compatibility alias for [`@StatefulAlgorithm`](@ref)."""
macro ProcessAlgorithm(ex)
    @warn "`@ProcessAlgorithm` is deprecated; use `@StatefulAlgorithm` instead." maxlog = 1
    return _stateful_algorithm_macro(ex)
end

export @StatefulAlgorithm, @ProcessAlgorithm, @init, @input, @inputs
